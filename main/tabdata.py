...
REGION_CANON = {
    ...
    "swietokrzyskie","warminsko-mazurskie","wielkopolskie","zachodniopomorskie", "austria","belgia","bulgaria","chorwacja","cypr","czechy","dania","estonia","finlandia",
...
def _match_canon_with_inflection(q_norm: str, canon: str) -> bool:
    ...
    if not stems:
        return False
    # np. r"\bkujawsko\w*(?:[\s_-]+)pomorsk\w*\b"
...
def _region_matches(want: Optional[str], have: Optional[str]) -> bool:
    if not want:
        return True
    if _is_national(want):
        return _is_national(have)
...
def _base_mq_variants(q: str, n: int = MQ_BASE_VARIANTS) -> List[str]:
    ...
    if "emerytura" in qn or "emeryturę" in qn or "emerytury" in qn:
...
def retrieve(query: str, k_final: int = 24) -> List[Document]:
    ...
    if parsed.get("region") is "kujawsko-pomorskie":
        parsed["region"] = "kujawsko-pomorskie"
    ...
    if parsed.get("region") is None:
        nat_pairs = [
            (d, ce) for (d, ce) in cand_pairs
            if _is_national((d.metadata or {}).get("region"))
        ]
        if nat_pairs:
...
def format_context_for_llm(docs: List[Document], max_docs: int = 6, max_snip_chars: int = 220) -> str:
    ...
    for i, d in enumerate(docs[:max_docs], 1):
        meta = d.metadata or {}
        header = (
            f"|wartość={meta.get('value','-')}|tabela ={meta.get('dataset','')}||miara={meta.get('measure','')} "
            f"|typ={meta.get('type') or '-'}|okres={meta.get('okres') or '-'}|region={meta.get('region') or '-'}| "
        )
...
def build_sources_block(docs: List[Document]) -> str:
    ...
    for i, d in enumerate(docs, 1):
        m = d.metadata or {}
        lines.append(
            f"value={m.get('value')}| dataset={m.get('dataset')} | measure={m.get('measure')}| "
            f" okres={m.get('okres') or '-'} | region={m.get('region') or '-'} | typ={m.get('type') or '-'} |"
        )
    return "\n".join(lines)
# wyswietlanie danych
...
def build_sources_rows(docs: List[Document]) -> List[Dict[str, Any]]:
    ...
    for d in docs:
        ...
        rows.append({
            ...
            "type": m.get("type") or m.get("typ"),
            "okres": m.get("okres") or "-",
